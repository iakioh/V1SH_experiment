...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='\\\\kfs\\krothe\\Windows Folders\\My Documents\\V1SH_experiment\\experiment\\code\\v0.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    ys = np.linspace(-spacing*(n_rows-1)/2, spacing*(n_rows-1)/2, n_rows)
    ...
    orientations = np.concatenate([oris_upper, oris_lower])
    ...
    while continueRoutine:
        ...
        if StopKey.status == STARTED and not waitOnFlip:
            theseKeys = StopKey.getKeys(keyList=['y','n','left','right','space'], ignoreKeys=["escape"], waitRelease=False)
    ...
    if StopKey.keys in ['', [], None]:  # No response was made
        StopKey.keys = None
    thisExp.addData('StopKey.keys',StopKey.keys)
    if StopKey.keys != None:  # we had a response
        thisExp.addData('StopKey.rt', StopKey.rt)
        thisExp.addData('StopKey.duration', StopKey.duration)
    thisExp.nextEntry()
